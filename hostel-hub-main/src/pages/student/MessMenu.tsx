    ...
    <DashboardLayout>
      <div className="space-y-8">
        <div className="animate-slide-up">
          <h1 className="text-3xl font-display font-bold mb-2">Mess Menu</h1>
          <p className="text-muted-foreground">Weekly meal schedule</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent rounded-2xl border border-primary/20 p-6 animate-fade-in">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground">Today's Menu</p>
              <h2 className="text-xl font-display font-bold">{today}</h2>
            </div>
          </div>
          {mockMessMenu
            .filter((menu) => menu.day === today)
            .map((menu) => (
              <div key={menu.day} className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/50">
                  <th className="text-left p-4 font-display font-semibold">Day</th>
              ...
              <tbody>
                {mockMessMenu.map((menu, index) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-muted-foreground">{menu.breakfast}</td>
                    <td className="p-4 text-sm text-muted-foreground">{menu.lunch}</td>
                    <td className="p-4 text-sm text-muted-foreground">{menu.snacks}</td>
                    <td className="p-4 text-sm text-muted-foreground">{menu.dinner}</td>
                  </tr>
                ))}
              </tbody>
            </table>
